...
/** Per-channel tolerance for "this pixel matches the colour of its own edge". */
 ...
 * outermost pixel. The two edges are independent and are never required to be
 * the same colour, so a page can be margined on one side, on both, or in two
 * different colours, and each side is read for what it is.
 *
 ...
 *
 * A page inset in ONE uniform background, which is every page the reference
 * bands were measured from, matched the left-most pixel at both edges already
 * and measures exactly what it measured before.
 */
 ...
 *
 * The trim keeps the pair it has always used: BOTH runs anchored on the
 * left-most pixel, which is the rule `rowMargins` carried before #255 and no
 * longer does. It is kept deliberately and it is not right. A reserved band on
 * the right is not recognised here, so its pixels are averaged into the
 * palette — the same synthetic art measures a mean luminance of 76.4 with the
 * band on the left and 110.0 with the band on the right, which is the near-white
 * failure above, half applied.
 *
 * It stays because every colour range in every shipped band was measured through
 * this trim, and moving it re-bases `valueMean`, `valueSpread`, `saturationMean`
 * and `hueBias` for all of them. Measured, by anchoring the right-hand run on
 * the right-hand pixel and re-reading the pages this repository holds:
 * `examples/dead-air` moves from 85.2 to 86.0 mean luminance, 73.2 to 73.5
 * spread, 0.263 to 0.261 saturation and 159.2 to 156.8 hue; the calm rhythm
 * fixture, whose art is full bleed, moves 209.6 to 210.4 and 0.173 to 0.171.
 * That is a colour re-basing with its own evidence to gather, not a side effect
 * an inset fix gets to have.
 */
function sampleColor(ctx: SKRSContext2D, width: number, artRows: readonly number[]): ColorSums {
  ...
  for (let index = 0; index < artRows.length; index += stride) {
    ...
    const left = edgeRun(row, width, 1, 0);
    const right = edgeRun(row, width, -1, 0);
