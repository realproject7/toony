...
export function encodeCanvas(canvas: Canvas, format: RasterFormat, quality?: number): Uint8Array {
  ...
  if (format === "jpeg") {
    return new Uint8Array(canvas.toBuffer("image/jpeg", quality ?? DEFAULT_JPEG_QUALITY));
  }
  return new Uint8Array(canvas.toBuffer("image/webp", quality ?? DEFAULT_WEBP_QUALITY));
}
