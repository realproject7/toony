...
function summary(name, pass, marker = "#", changes = {}) {
  ...
  return Object.entries(fields)
    .map(([key, value]) => `${name}:test: ${marker} ${key} ${value}`)
    .join("\n");
}
...
test("package identities cannot be masked by a replacement task or a larger total", () => {
  assert.throws(
    () => checkTestTasks(["@fixture/a", "replacement"], baseline),
    /missing test task: fixture-b/,
  );
  assert.throws(
    () => checkTestResults(`${summary(names[0], 100)}\n${summary(names[1], 1)}`, names, baseline),
    /test reduction: fixture-b/,
  );
  assert.throws(
    () => checkTestResults(summary(names[0], 100), names, baseline),
    /unverifiable.*fixture-b/,
  );
});
...
test("a deliberate count reduction or task removal requires a recorded reason", () => {
  const reduced = {
    ...baseline,
    reductions: {
      "fixture-b": { minimum: 1, reason: "Removed duplicate assertion; reviewed in the PR" },
    },
  };
  assert.match(
    checkTestResults(`${summary(names[0], 3)}\n${summary(names[1], 1)}`, names, reduced).join("\n"),
    /declared reduction/,
  );
  reduced.reductions["fixture-b"].minimum = 0;
  assert.match(
    checkTestResults(summary(names[0], 3), [names[0]], reduced).join("\n"),
    /fixture-b -> 0/,
  );
...
test("deleting a real package test script is detected before the test command runs", () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: names[1], scripts: {} }),
    );
    assert.throws(
      () => checkTestTasks(testPackages(root), baseline),
      /missing test task: fixture-b/,
    );
  } finally {
  ...
  try {
    ...
    for (const stage of ["install", "check", "build", "test"]) {
      const result = spawnSync(
        ...
        [
          "-c",
          '. "$1"; run_stage "$2" bash -c \'echo "compiler diagnostic TS5083"; exit 7\'',
          "control",
          join(scripts, "release-stage.sh"),
          stage,
        ],
        {
          encoding: "utf8",
          env: { ...process.env, CLONE: clone, LOGS: logs, V: "24" },
        },
      );
    ...
    const result = spawnSync(
      ...
      [
        "-c",
        '. "$1"; run_stage test bash -c \'echo original-cause; rm -rf "$CLONE"; exit 1\'',
        "control",
        join(scripts, "release-stage.sh"),
      ],
      {
